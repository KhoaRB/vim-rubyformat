...
function! GeneralBeautifyBefore()
	...
	" IF NOT SURROUNDED BY QUOTES ET CETERA OR NOT AN = SIGN
	" AFTER IT
...
function! RubyFormat()
	...
	" IN THE FILE OR THERE ISN'T ANOTHER `}` OR THE
	" KEYWORD `end` ON THE LINE BELOW IT
	g/}.\{-\}\n\(\n\+\)\@!\([^\n]\)\@=/s/^\(.*\)}\(.\{-\}\)\(.\{-\}\n.\{-\}\(}\|\<end\>\)\)\@!\s\{-\}$/\1}\2\r/ge
...
function! ERubyFormat()
	" SAVE CURSOR POSITION IN WINDOW BEFORE FORMAT
	let l:winview = winsaveview()

	call GeneralBeautifyBefore()

	" ADD SPACING TO ANYTHING THAT STARTS OR ENDS WITH `<%` OR `%>`
	g!/-%>/s/\(\S\)%>/\1 %>/ge
	g!/<%=\|<%-\|<%#/s/<%\(\S\)/<% \1/ge
	%s/<%#\(\S\)/<%# \1/ge
	%s/<%=\(\S\)/<%= \1/ge
...
" TEMPORARY MINIMAL SUPPORT FOR ERB FILES (SIMPLE SPACING UNTIL COMPLETE
" REFACTOR)
au FileType eruby nnoremap <buffer> ff :silent! call ERubyFormat()<CR>
...
if g:rubyformat_on_save > 0
	...
	autocmd FileType eruby autocmd BufWritePre <buffer> :silent! call ERubyFormat()
endif
